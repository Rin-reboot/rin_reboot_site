...
export default async function PostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="blog-main"><Breadcrumbs items={[{ name: "Home", href: "/" }, { name: "Blog", href: "/blog/" }, { name: post.category, href: `/blog/category/${encodeURIComponent(post.category)}/` }, { name: post.title, href: `/blog/${post.slug}/` }]} /><article className="blog-article"><header><p className="article-category"><Link href={`/blog/category/${encodeURIComponent(post.category)}/`}>{post.category}</Link></p><h1>{post.title}</h1><p className="article-description">{post.description}</p><div className="article-dates"><time dateTime={post.publishedAt}>公開 {post.publishedAt}</time>{post.updatedAt && <time dateTime={post.updatedAt}>更新 {post.updatedAt}</time>}</div><ul aria-label="記事のタグ">{post.tags.map((tag) => <li key={tag}><Link href={`/blog/tag/${encodeURIComponent(tag)}/`}>#{tag}</Link></li>)}</ul></header>{/* biome-ignore lint/security/noDangerouslySetInnerHtml: Markdown is converted without raw HTML support from trusted repository content. */}<div className="article-body" dangerouslySetInnerHTML={{ __html: html }} /></article><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd).replace(/</g, "\\u003c") }} /></main>;
}
